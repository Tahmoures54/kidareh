...
export async function searchProductsService(params: SearchParams): Promise<SearchResult> {
  const cacheKey = CacheKeys.search(hashParams({
    limit: params.limit,
...
function searchProductsFromDb(params: SearchParams): SearchResult {
  const { limit, cursor, q, category, city, province, scope, sort, onlyAvailable, minPrice, maxPrice, radiusKm, lat, lng } = params;
  const where: string[] = [];
  ...
  if (q && q.trim()) {
    ...
    if (!usedFts) {
      ...
      where.push(`(p.name LIKE ? ESCAPE '\\' OR p.description LIKE ? ESCAPE '\\' OR s.name LIKE ? ESCAPE '\\')`);
      engine = "like";
    }
  }
  if (category) { whereValues.push(category); where.push(`p.category = ?`); }
  if (scope === "city" && city) { whereValues.push(city); where.push(`p.city = ?`); }
  if (scope === "province" && province) { whereValues.push(province); where.push(`p.province = ?`); }
  if (onlyAvailable) where.push(`p.status IN ('موجود', 'فقط ۱ عدد')`);
  if (minPrice != null) { whereValues.push(minPrice); where.push(`p.price >= ?`); }
  if (maxPrice != null) { whereValues.push(maxPrice); where.push(`p.price <= ?`); }
  if (cursor?.id != null) { whereValues.push(cursor.id); where.push(`p.id < ?`); }
  ...
  const selectDistanceValues: number[] = hasCoords ? [lat as number, lng as number, lat as number] : [];
  let radiusClause = "";
  ...
  if (sort === "cheapest") orderBy = `p.price ASC, p.created_at DESC`;
  else if (sort === "nearest" && hasCoords) orderBy = `(distance IS NULL), distance ASC, p.created_at DESC`;
  else if (engine === "fts5" && (sort === "relevance" || !sort)) {
    const ids = searchProductIdsFts(q!, 1000);
    if (ids.length) orderBy = `${featuredFirst}, CASE p.id ${ids.map((id, i) => `WHEN ${id} THEN ${i}`).join(" ")} ELSE 9999 END, p.created_at DESC`;
  }

  const finalWhere = [...where];
  if (radiusClause) finalWhere.push(radiusClause);
  const whereClause = finalWhere.length ? `WHERE ${finalWhere.join(" AND ")}` : "";
  const sqlParams: unknown[] = [...selectDistanceValues, ...whereValues, ...radiusValues];
  const limitPlusOne = limit + 1;
  const sql = `
    SELECT
      p.id, p.name, p.price, p.status, p.badge, p.image_url,
      p.created_at, p.updated_at, p.store_id,
      s.user_id AS store_user_id, s.phone AS store_phone,
      s.lat, s.lng, p.category, p.views, p.city, p.province,
      s.name AS store_name, s.city AS store_city, s.province AS store_province,
      s.image_url AS store_image_url, s.has_business_license,
      4.5 AS rating, ${distanceExpr} AS distance
    FROM products p
  ...
  try {
    ...
    const countSql = `SELECT COUNT(*) AS total FROM products p LEFT JOIN stores s ON s.id = p.store_id ${whereClause}`;
