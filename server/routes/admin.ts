  ...
  try {
    ...
    let query = `
      SELECT 
        ...
        s.has_business_license, s.blue_tick_expires_at,
    ...
    if (banned === "true") countQuery += " AND is_banned = 1";
    else if (banned === "false") countQuery += " AND is_banned = 0";
    ...
    return res.json({
      users: users.map((u: any) => ({
        ...
        blue_tick_active: u.blue_tick_expires_at
          ? new Date(u.blue_tick_expires_at) > new Date()
          : false,
      })),
      pagination: {
        page: pageNum,
...
router.post("/users/:id/verify", (req: AuthRequest, res: Response): void => {
  try {
    ...
    if (!store) {
      res.status(404).json({ error: "این کاربر هنوز فروشگاهی ثبت نکرده است." });
      return;
    }
    db.prepare(
      "UPDATE stores SET has_business_license = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?"
    ).run(verify ? 1 : 0, userId);
    logger.info(`${verify ? "✅" : "❌"} Store ${store.id} verification changed by admin ${req.user?.id}`);
    res.json({ success: true, message: verify ? "فروشگاه با موفقیت تایید شد." : "تاییدیه فروشگاه لغو شد." });
  } catch (error) {
...
router.post("/users/:id/ban", (req: AuthRequest, res: Response): void => {
  try {
    ...
    if (userId === String(req.user?.id)) {
      res.status(400).json({ error: "شما نمی‌توانید حساب کاربری خودتان را مسدود کنید." });
      return;
    }
    const user = db.prepare("SELECT phone FROM users WHERE id = ?").get(userId) as any;
    if (!user) {
...
router.post("/users/:id/unban", (req: AuthRequest, res: Response): void => {
  try {
    ...
    res.json({ success: true, message: "کاربر رفع مسدودیت شد." });
  } catch (error) {
...
router.post("/users/:id/role", (req: AuthRequest, res: Response): void => {
  try {
    ...
    if (userId === String(req.user?.id) && role !== "admin") {
      res.status(400).json({ error: "شما نمی‌توانید نقش خود را از admin تغییر دهید" });
      return;
    }
...
router.get("/products/pending", (_req: AuthRequest, res: Response) => {
  try {
...
router.post("/products/:id/approve", (req: AuthRequest, res: Response): void => {
  try {
    ...
    db.prepare("UPDATE products SET moderation_status = 'approved', updated_at = CURRENT_TIMESTAMP WHERE id = ?").run(id);
...
/** List all stores with follower count & blue tick status */
  ...
  try {
    const stores = db.prepare(`
      SELECT s.*,
             ...
             COUNT(sf.id) as follower_count,
             CASE WHEN s.blue_tick_expires_at > datetime('now') THEN 1 ELSE 0 END as has_blue_tick
      FROM stores s
    ...
    res.json({
      stores: stores.map((s: any) => ({
        ...
        has_blue_tick: !!s.has_blue_tick,
      })),
    });
  } catch (error) {
    logger.error("Fetch stores error:", error);
    res.status(500).json({ error: "خطا در دریافت لیست فروشگاه‌ها" });
  }
});
...
router.post("/stores/:id/toggle-blue-tick", (req: AuthRequest, res: Response): void => {
  ...
  try {
    ...
    if (hasBlueTick) {
      db.prepare("UPDATE stores SET blue_tick_expires_at = NULL WHERE id = ?").run(storeId);
      res.json({ blue_tick: false, message: "تیک آبی فروشگاه حذف شد." });
    } else {
      const expires = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString();
      db.prepare("UPDATE stores SET blue_tick_expires_at = ? WHERE id = ?").run(expires, storeId);
      res.json({ blue_tick: true, expires, message: "تیک آبی فروشگاه فعال شد." });
    }
    logger.info(`🔵 Blue tick toggled for store ${storeId} by admin ${req.user?.id}`);
  } catch (error) {
    logger.error("Toggle blue tick error:", error);
    res.status(500).json({ error: "خطا در تغییر وضعیت تیک آبی." });
  }
});
...
router.get("/settings", (_req: AuthRequest, res: Response) => {
  try {
    const settings = db.prepare("SELECT key, value, description FROM settings").all() as any[];
...
/** Get System Info */
router.get("/system-info", (_req: AuthRequest, res: Response) => {
  try {
    const info = {
