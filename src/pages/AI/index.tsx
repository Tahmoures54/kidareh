...
import { ArrowRight, Bot, Cpu, Send, Loader2, Search, MapPin, ShieldCheck } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#f8fafc] dark:bg-gray-950 flex flex-col relative overflow-hidden transition-colors" dir="rtl">
      ...
      <header className="sticky top-0 z-30 bg-white/80 dark:bg-gray-950/80 backdrop-blur-2xl border-b border-gray-200/50 dark:border-gray-800/50 px-4 pt-[max(1rem,env(safe-area-inset-top))] pb-3 shadow-sm shadow-gray-900/5">
        <div className="flex items-center gap-3 max-w-3xl mx-auto">
          <button onClick={() => navigate(-1)} aria-label="بازگشت" className="w-10 h-10 bg-gray-100/80 dark:bg-gray-900 rounded-full flex items-center justify-center active:scale-90 transition-transform">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-1 mt-0.5"><span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" /><span className="text-[10px] text-gray-500 dark:text-gray-400 font-bold">جستجو در فروشگاه‌های ثبت‌شده</span></div>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="max-w-3xl mx-auto">
          {state.messages.length <= 1 && !state.loading && (
            <div className="mb-5 grid grid-cols-3 gap-2">
              <div className="rounded-2xl border border-gray-200/80 dark:border-gray-800 bg-white/80 dark:bg-gray-900/80 p-3"><Search className="w-4 h-4 text-violet-500 mb-2" /><p className="text-[11px] font-bold text-gray-800 dark:text-gray-200">پیدا کردن کالا</p><p className="text-[9px] text-gray-500 mt-1">از موجودی ثبت‌شده</p></div>
              <div className="rounded-2xl border border-gray-200/80 dark:border-gray-800 bg-white/80 dark:bg-gray-900/80 p-3"><MapPin className="w-4 h-4 text-violet-500 mb-2" /><p className="text-[11px] font-bold text-gray-800 dark:text-gray-200">نزدیک‌ترین گزینه</p><p className="text-[9px] text-gray-500 mt-1">با موقعیت شما</p></div>
              <div className="rounded-2xl border border-gray-200/80 dark:bg-gray-900/80 border-gray-200/80 p-3"><ShieldCheck className="w-4 h-4 text-violet-500 mb-2" /><p className="text-[11px] font-bold text-gray-800 dark:text-gray-200">نتیجه واقعی</p><p className="text-[9px] text-gray-500 mt-1">بدون ساختن فروشگاه</p></div>
            </div>
          )}
      ...
      <div className="relative z-20 bg-gradient-to-t from-white via-white dark:from-gray-950 dark:via-gray-950 to-transparent pt-6 pb-[max(1rem,env(safe-area-inset-bottom))] px-4">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-center text-[9px] font-bold text-gray-400 dark:text-gray-600 mt-3">نتایج بر اساس اطلاعات ثبت‌شده فروشگاه‌هاست؛ موجودی را قبل از مراجعه تأیید کنید.</p>
        </div>
      </div>
    </div>
  );
