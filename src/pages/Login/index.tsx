...
import { Loader2, LockKeyholeOpen, ShieldCheck, Lock, CheckCircle2 } from "lucide-react"; // 🟢 CheckCircle2 اضافه شد
...
export default function Login() {
  ...
  if (state.authLoading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900">
      ...
      <span className="text-sm font-bold text-slate-400">در حال بررسی...</span>
    </div>
  );
  ...
  return (
    <div className="min-h-[100dvh] w-full max-w-md mx-auto bg-white dark:bg-[#0B0F19] flex flex-col relative overflow-hidden font-sans shadow-2xl" dir="rtl">
      ...
      <MiniHeader 
        ...
        onBack={() => { setters.setStep("phone"); setters.setOtp(""); setters.setError(""); }} 
      />
      ...
      <main className="flex-1 flex flex-col justify-center px-8 w-full z-10 py-10">
        ...
        <div className="mb-10 text-center">
          ...
          >
            ...
            <div className="absolute inset-0 bg-white/20 rounded-[32px] blur-md -z-10" />
          </motion.div>
          ...
          >
            {state.isSuccess ? "ورود موفق!" : state.step === "phone" ? "خوش آمدید!" : "تایید شماره"}
          </motion.h2>
          ...
          >
            {state.isSuccess 
              ? "در حال آماده‌سازی حساب کاربری شما..." 
              : state.step === "phone" 
                ? "شماره موبایلت رو وارد کن تا یه کد برات بفرستیم. سریع و امن!" 
                : "کد ۵ رقمی پیامک شده رو برامون بنویس تا وارد بشی."}
          </motion.p>
        </div>
        ...
        <AnimatePresence mode="wait">
          {state.step === "phone" ? (
            <StepPhone 
              key="phone-step" phone={state.phone} setPhone={setters.setPhone} 
              onSubmit={actions.onPhoneSubmit} loading={state.loading} error={state.error}
            />
          ) : (
            <StepOTP 
              key="otp-step" phone={state.phone} otp={state.otp} 
              onOtpChange={actions.onOtpChange} onOtpKey={actions.onOtpKey} 
              onOtpPaste={actions.onOtpPaste} onSubmit={actions.onOtpSubmit} 
              onResend={actions.onPhoneSubmit} timer={state.timer} 
              loading={state.loading} error={state.error} isSuccess={state.isSuccess} // 🟢 پراپ جدید
              onChangePhone={() => { setters.setStep("phone"); setters.setOtp(""); setters.setError(""); }}
            />
          )}
        </AnimatePresence>
      </main>
