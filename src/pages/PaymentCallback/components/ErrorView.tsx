...
const ErrorView = ({ msg, code, retry, retryCount, onSupport }: Props) => (
  ...
  >
    ...
    <h2 className="text-xl font-black text-gray-900 dark:text-white mb-4 tracking-tight">پرداخت ناموفق بود</h2>
    ...
    <div className="bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 rounded-2xl p-4 text-right mb-5 flex items-start gap-3 shadow-sm">
      ...
      <p className="text-[13px] text-rose-800 dark:text-rose-300 font-bold leading-relaxed">{msg}</p>
    </div>
    ...
    <div className="flex gap-3 mb-2">
      <button onClick={onSupport} className="flex-1 h-14 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-2xl font-black text-sm flex items-center justify-center gap-1.5 hover:bg-gray-200 dark:hover:bg-gray-700 active:scale-[0.98] transition-all shadow-sm">
        <LifeBuoy className="w-5 h-5" /> پشتیبانی
      </button>
      <button onClick={retry} disabled={retryCount >= MAX_RETRY} className="flex-[1.5] h-14 bg-indigo-600 dark:bg-indigo-500 text-white rounded-2xl font-black text-sm flex items-center justify-center gap-2 disabled:opacity-50 active:scale-[0.98] transition-all shadow-xl shadow-indigo-600/20">
    ...
    <p className="text-[10px] text-gray-400 dark:text-gray-500 font-bold h-4">
      ...
      {retryCount >= MAX_RETRY && "تعداد تلاش به پایان رسید."}
    </p>
  </motion.div>
);
