      ...
      {open && (
        <>
          ...
          >
            ...
            <h3 className="text-base font-black text-gray-900 dark:text-white mb-4 flex items-center gap-2"><SlidersHorizontal className="w-5 h-5 text-teal-600" /> مرتب‌سازی فروشگاه‌ها</h3>
            <div className="space-y-2">
              {SORT_OPTIONS.map((opt) => (
                <button
                  key={opt.id} onClick={() => { onChange(opt.id); onClose(); }}
                  className={`w-full flex items-center justify-between px-4 py-3.5 rounded-2xl text-sm font-bold transition-all ${value === opt.id ? "bg-teal-50 text-teal-800 border border-teal-100 shadow-sm" : "bg-gray-50 text-gray-700 border border-transparent hover:bg-gray-100"}`}
                >
