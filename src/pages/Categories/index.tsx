...
import {
  ArrowRight, Search, X, LayoutGrid, Package, Sparkles, Layers
} from "lucide-react";
...
export default function Categories() {
  ...
  const totalTypes = useMemo(
    () => filtered.reduce((s, g) => s + g.types.length, 0),
    [filtered]
  );
      ...
      <header className="sticky top-0 z-30 bg-white/80 dark:bg-gray-950/80 backdrop-blur-2xl border-b border-gray-200/60 dark:border-gray-800/60 px-4 pt-[max(1rem,env(safe-area-inset-top))] pb-4 shadow-sm shadow-gray-900/5">
        ...
        <div className="relative group">
          <div className="absolute inset-y-0 right-0 pl-3 flex items-center pointer-events-none pr-4">
            <Search className="w-5 h-5 text-gray-400 group-focus-within:text-indigo-600 dark:group-focus-within:text-indigo-400 transition-colors" />
          </div>
          <input
            type="text"
            placeholder="کالای مورد نظرتان در کدام دسته است؟"
            value={query}
            onChange={e => setQuery(e.target.value)}
            className="w-full bg-gray-100 dark:bg-gray-900 border border-transparent rounded-2xl pl-12 pr-11 py-3.5 text-sm font-bold text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:bg-white dark:focus:bg-gray-800 focus:border-indigo-500/50 dark:focus:border-indigo-400/50 focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none"
          />
          <AnimatePresence>
            {query && (
              <motion.button
          ...
          {filtered.length === 0 ? (
            ...
            >
              <div className="relative w-24 h-24 mx-auto mb-6">
                <div className="absolute inset-0 bg-indigo-500/10 dark:bg-indigo-500/20 rounded-full animate-ping" style={{ animationDuration: '3s' }} />
              ...
              <h3 className="text-lg font-black text-gray-900 dark:text-white mb-2">دسته‌ای پیدا نشد</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">لطفاً عبارت دیگری را جستجو کنید.</p>
              <button 
                onClick={() => setQuery("")}
                className="mt-6 px-6 py-2.5 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-xl font-bold text-sm shadow-md active:scale-95 transition-transform"
              >
                پاک کردن جستجو
              </button>
            </motion.div>
            ...
            <motion.div key="list" layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
              <AnimatePresence>
                {!query && (
                  ...
                  >
                    {[
                      { label: "گروه اصلی", value: filtered.length, icon: LayoutGrid, bg: "bg-indigo-50 dark:bg-indigo-500/10", color: "text-indigo-600 dark:text-indigo-400" },
                      { label: "زیردسته", value: totalTypes, icon: Package, bg: "bg-teal-50 dark:bg-teal-500/10", color: "text-teal-600 dark:text-teal-400" },
                      { label: "محصولات", value: "هزاران+", icon: Sparkles, bg: "bg-amber-50 dark:bg-amber-500/10", color: "text-amber-600 dark:text-amber-400" },
                    ].map(s => (
                      <div key={s.label} className={`${s.bg} rounded-3xl p-4 flex flex-col items-center justify-center gap-2 border border-white/50 dark:border-gray-800`}>
