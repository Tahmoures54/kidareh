  ...
  return (
    <>
      <div className="space-y-4 mb-8">
        <div className="flex flex-wrap justify-between items-start gap-3">
          <div className="flex flex-wrap gap-2">
            {product.badge && <span className={`px-3 py-1.5 rounded-xl text-xs font-black shadow-sm ${getBadgeStyle(product.badge)}`}>{product.badge}</span>}
            {product.category && <span className="px-3 py-1.5 bg-[var(--brand-primary)]/10 text-[var(--brand-primary)] border border-[var(--brand-primary)]/20 text-xs font-bold rounded-xl">{product.category}</span>}
          </div>
          <span className={`px-4 py-1.5 rounded-xl text-xs font-black flex items-center gap-1.5 shrink-0 border ${isAvailable ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/20" : "bg-[var(--bg-tertiary)] text-[var(--text-muted)] border-[var(--border-light)]"}`}>
      ...
      <div className="bg-gradient-to-br from-[var(--brand-primary)]/10 to-[var(--brand-secondary)]/5 border border-[var(--brand-primary)]/20 rounded-3xl p-6 mb-8 flex justify-between items-center relative overflow-hidden group">
        ...
        <div>
          <p className="text-[var(--brand-primary)] text-xs font-black uppercase tracking-widest mb-1 opacity-80">قیمت کالا</p>
      ...
      <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200/50 dark:border-amber-800/30 rounded-3xl p-5 flex items-start gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-xs text-amber-800/80 dark:text-amber-400/70 leading-relaxed font-bold">پیش از اطمینان از صحت کالا، از پرداخت هرگونه بیعانه خودداری کنید و معامله را ترجیحاً حضوری انجام دهید.</p>
        </div>
      </div>
    </>
  );
