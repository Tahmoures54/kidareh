...
export class GlobalErrorBoundary extends Component<Props, State> {
  ...
  render() {
    if (this.state.hasError && this.state.error) {
      // If error count is too high, show permanent error
      if (this.state.errorCount > 3) {
        return (
          <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-slate-950 p-6" dir="rtl">
            <div className="w-20 h-20 bg-red-100 dark:bg-red-900/30 rounded-3xl flex items-center justify-center mb-6">
              <AlertTriangle className="w-10 h-10 text-red-600 dark:text-red-400" />
            </div>
            <h1 className="text-2xl font-black text-gray-900 dark:text-white mb-2">خطای سیستمی</h1>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-8 text-center max-w-sm">
              متأسفانه مشکلی در برنامه پیش آمد. لطفاً بعداً دوباره تلاش کنید یا با تیم پشتیبانی تماس بگیرید.
            </p>
            ...
            {process.env.NODE_ENV === 'development' && (
              <div className="mt-8 p-4 bg-red-50 dark:bg-red-950/30 rounded-xl max-w-md overflow-auto">
                <p className="text-xs font-bold text-red-600 dark:text-red-400 mb-2">جزئیات خطا:</p>
                <pre className="text-xs text-red-700 dark:text-red-300 whitespace-pre-wrap word-break">
      ...
      return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-slate-950 p-6" dir="rtl">
          ...
          <h1 className="text-2xl font-black text-gray-900 dark:text-white mb-2">خطا در بارگذاری</h1>
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-8 text-center max-w-sm">
          ...
          {process.env.NODE_ENV === 'development' && (
            <div className="mt-8 p-4 bg-yellow-50 dark:bg-yellow-950/30 rounded-xl max-w-md overflow-auto">
              <p className="text-xs font-bold text-yellow-600 dark:text-yellow-400 mb-2">جزئیات خطا:</p>
