...
import { iranCities, IranCity } from '../../../../data/processed/iranCities';
...
export default function CitySearchComboBox({ value, onChange, placeholder = 'شهر...' }: Props) {
  ...
  useEffect(() => {
    ...
    const filtered = iranCities.filter(c => c.name.toLowerCase().includes(term) || c.province.toLowerCase().includes(term));
  ...
  function onKey(e: KeyboardEvent<HTMLInputElement>) {
    ...
    else if (e.key === 'Enter') { e.preventDefault(); const it = items[highlight]; if (it) { onChange(it.name, it.display, it.province); setOpen(false); setQ(''); } }
          ...
          {items.length === 0 ? (
            ...
            items.map((it, idx) => (
              <button
                ...
                onClick={() => { onChange(it.name, it.display, it.province); setOpen(false); setQ(''); }}
