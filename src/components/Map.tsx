...
// ==========================================
// 1. طراحی مارکرهای اختصاصی (بدون نیاز به عکس خارجی)
// ==========================================

// مارکر موقعیت شما (آبی با انیمیشن تپش)
...
// مارکر فروشگاه‌ها (پین بنفش با آیکون)
const createStoreIcon = (isHighlight = false) => L.divIcon({
  className: 'custom-store-marker',
  html: `
    <div class="relative group cursor-pointer -ml-4 -mt-8">
      <div class="w-8 h-8 ${isHighlight ? 'bg-rose-500' : 'bg-indigo-600'} rounded-t-full rounded-bl-full rotate-45 flex items-center justify-center shadow-lg border-2 border-white transition-transform group-hover:scale-110 group-hover:shadow-xl">
...
// ==========================================
// 3. کنترل‌کننده نقشه (برای زوم و حرکت)
// ==========================================
function MapController({ center, zoomTrigger }: { center: Location, zoomTrigger: number }) {
  const map = useMap();
  
  useEffect(() => {
    if (zoomTrigger > 0) {
      // حرکت نرم و پروازی به سمت کاربر
...
// ==========================================
// 4. کامپوننت اصلی نقشه
// ==========================================
export default function Map({ center, results, height = '65vh' }: MapProps) {
  ...
  return (
    <div className="relative w-full rounded-[2rem] overflow-hidden border border-gray-200/60 shadow-lg bg-gray-50 z-0" style={{ height }}>
      
      {/* دکمه‌های کنترلی شناور روی نقشه */}
      ...
      <MapContainer 
        ...
        zoomControl={false} // حذف دکمه‌های +- پیش‌فرض
        attributionControl={false} // مخفی کردن نوشته واترمارک لیفلت برای ظاهر کلین‌تر
      >
        <MapController center={center} zoomTrigger={zoomTrigger} />
        
        {/* لایه نقشه - استفاده از تایل‌های روشن‌تر */}
        <TileLayer
        ...
        {/* مارکر فروشگاه‌ها و کالاها */}
        {results.map((result, index) => {
          // در یک اپلیکیشن واقعی باید lat و lng واقعی از دیتابیس بیایند
          // اینجا به صورت امن از مقادیر تستی استفاده می‌کنیم اگر مقادیر واقعی نبود
          const lat = result.latitude || center.lat + (Math.sin(index) * 0.015);
          const lng = result.longitude || center.lng + (Math.cos(index) * 0.015);
          
          return (
            <Marker 
              key={result.id} 
              position={[lat, lng]}
              icon={createStoreIcon(!!result.badge)} // اگر برچسب داشت قرمز میشه
            >
              <Popup className="store-popup" closeButton={false}>
                <div className="w-[200px] flex flex-col p-1" dir="rtl">
                  ...
                  <div className="w-full h-24 rounded-xl overflow-hidden mb-2 bg-gray-50 relative">
                    <img 
                      src={result.image || `https://picsum.photos/seed/${result.id}/200/100`} 
                      alt={result.name}
                  ...
                  <div className="px-1">
                    ...
                    <p className="text-sm font-black text-indigo-600 mb-3 mt-2">
                      {typeof result.price === 'number' ? result.price.toLocaleString('fa-IR') : result.price} <span className="text-[9px] text-gray-400">تومان</span>
                    </p>
